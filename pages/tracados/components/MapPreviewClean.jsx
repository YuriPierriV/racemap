...
const MapPreviewClean = ({ 
  ...
  const mapId = useMemo(() => `map-clean-${Math.random().toString(36).substr(2, 9)}`, []);
    ...
    const timeoutId = setTimeout(() => {
      Promise.all([
        ...
        if (showStartOnly && normalizedPoints.length === 1) {
          ...
          LeafletModule.marker([normalizedPoints[0].lat, normalizedPoints[0].lng], { 
          ...
          const startMarker = LeafletModule.marker([normalizedPoints[0].lat, normalizedPoints[0].lng], { 
            icon: startFlagIcon,
          ...
          if (onStartPositionChange) {
            startMarker.on('dragend', function(event) {
              ...
              normalizedPoints.forEach(point => {
                ...
                const a = Math.sin(dLat/2) * Math.sin(dLat/2) +
                          Math.cos(position.lat * Math.PI / 180) * Math.cos(point.lat * Math.PI / 180) *
                          Math.sin(dLng/2) * Math.sin(dLng/2);
                const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
              ...
              onStartPositionChange(closestPoint.lat, closestPoint.lng, closestPoint.index);
            });
          }
          ...
          if (normalizedPoints.length > 1 && showCurrentPosition) {
            ...
            LeafletModule.marker(
              [normalizedPoints[normalizedPoints.length - 1].lat, normalizedPoints[normalizedPoints.length - 1].lng], 
  ...
  }, [points, direction, showStartOnly, showCurrentPosition, onStartPositionChange, isClient, mapStyle, effectiveTheme]);
