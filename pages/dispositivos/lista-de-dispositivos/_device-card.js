...
import { CheckCircle, CirclePause, Ellipsis, FastForward, Gauge, Loader2, Play, Trash2, XCircle } from "lucide-react"; // Ícones
import { translateMode, useChangeMode } from "pages/comunication/ModeSelector";
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="icon" size="icon"><Ellipsis></Ellipsis></Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent >
                ...
                <DropdownMenuGroup>
                    ...
                    <DropdownMenuItem onClick={() => changeMode(speed)}>
...
export default function CardDevice({ device, filter }) {
    const { gpsStatus, mode, changeMode, handleCheckGpsStatus } = useGpsStatus(device.chip_id);
    ...
    return (
        <Card key={device.id}>
            <CardHeader>
                <CardTitle>
                    <div className="flex justify-between items-center">
                        {device.chip_id}
                        <DropdownMenuMode deviceId={device.chip_id} changeMode={changeMode}></DropdownMenuMode>
                    </div>
                </CardTitle>
                <CardDescription>
            ...
            <CardContent>
                <p className="text-gray-600">Detalhes do dispositivo podem ser adicionados aqui.</p>
            </CardContent>
            <CardFooter>
                <p className="text-gray-500">
                    <strong>Criado em:</strong> {new Date(device.created_at).toLocaleString()}
                </p>
            </CardFooter>
        </Card>
    );
