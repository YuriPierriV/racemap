...
export const useGpsData = (deviceChipId, options = {}) => {
  ...
  const processGpsMessage = useCallback(
    (message) => {
      try {
        ...
        if (
          previousGpsConnectedRef.current !== null &&
  ...
  const getTrackStats = useCallback(() => {
    ...
    const duration =
      trackPoints.length > 1
        ? (trackPoints[trackPoints.length - 1].timestamp -
    ...
    for (let i = 1; i < trackPoints.length; i++) {
      ...
      const Δφ =
        ((trackPoints[i].lat - trackPoints[i - 1].lat) * Math.PI) / 180;
      const Δλ =
        ((trackPoints[i].long - trackPoints[i - 1].long) * Math.PI) / 180;
  ...
  return {
    ...
    // Dados extras
    currentPosition:
      gpsData.lat && gpsData.long ? [gpsData.lat, gpsData.long] : null,
